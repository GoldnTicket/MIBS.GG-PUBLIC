// MIBS.GG-PUBLIC/server.js - PEEWEE DROP FIX
// ✅ Reduced drop VALUE by 5x
// ✅ Reduced drop COUNT by 5x
...
// ✅ FIX: Reduced drop value by 5x
function calculateBountyDrop(marble, C) {
  // Original: lengthScore * dropValueMultiplier
  // Fixed: Divide by 5
  const totalValue = (marble.lengthScore * C.collision.dropValueMultiplier) / 5;
  const bountyValue = marble.bounty || 1;
  return { totalValue, bountyValue };
}

// ✅ FIX: Reduced drop count by 5x (divide by 50 instead of 10)
function calculateDropDistribution(totalValue, C) {
  // Original: totalValue / 10
  // Fixed: Divide by 50 (5x fewer drops)
  const numDrops = Math.floor(totalValue / 50);
  const valuePerDrop = numDrops > 0 ? totalValue / numDrops : 0;
  return { numDrops: Math.max(1, numDrops), valuePerDrop };
}
...
function killMarble(marble, killerId) {
  ...
  // ✅ Limit max coins spawned
  const coinsToSpawn = Math.min(dropDist.numDrops, MAX_COINS - gameState.coins.length, 20);
  
  console.log(`💀 Marble killed: ${marble.name} | Drops: ${coinsToSpawn} coins (was ${dropDist.numDrops})`);
  ...
  for (let i = 0; i < coinsToSpawn; i++) {
    ...
    gameState.coins.push({
      ...
      growthValue: Math.floor(dropDist.valuePerDrop) || 2,  // ✅ Reduced base value
...
server.listen(PORT, () => {
  console.log(`╔═══════════════════════════════════╗`);
  console.log(`║   MIBS.GG SERVER - PEEWEE FIX     ║`);
  console.log(`╠═══════════════════════════════════╣`);
  console.log(`║ Port: ${PORT.toString().padEnd(28)}║`);
  console.log(`║ Version: ${gameConstants.version.padEnd(23)}║`);
  console.log(`║ Drops: 5x REDUCED                 ║`);
  console.log(`╚═══════════════════════════════════╝`);
